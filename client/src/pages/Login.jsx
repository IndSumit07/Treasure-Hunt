...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    const loadingToast = showToast.loading("Signaling Google fleet...");
    try {
      const { error } = await signInWithGoogle();
      if (error) throw error;
      showToast.dismiss(loadingToast);
      showToast.success("Signal received! Redirecting...");
    } catch (error) {
      showToast.dismiss(loadingToast);
  ...
  const handleGithubLogin = async () => {
    const loadingToast = showToast.loading("Signaling GitHub fleet...");
    try {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const loadingToast = showToast.loading('Verifying credentials...');
    try {
          ...
          {!showEmailForm ? (
            ...
            <div className="animate-fade-in">
              <button 
                ...
                className="mb-6 flex items-center text-sm text-gray-500 hover:text-treasure-bronze transition-colors font-medium"
              >
